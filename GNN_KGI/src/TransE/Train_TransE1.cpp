...
public:
    ...
    void run(int n_in,double rate_in,double margin_in,int method_in, string root)
    {
        n = n_in;
        rate = rate_in;
        margin = margin_in;
    ...
    void bfgs(std::string root)
    {
        ...
        int batchsize = fb_h.size()/nbatches;
            for (int epoch=0; epoch<nepoch; epoch++)
            {
                ...
                string relation2vecFile = root+"/relation2vec.";
                string entity2vecFile = root+"/entity2vec.";
                FILE* f2 = fopen((relation2vecFile+version).c_str(),"w");
                FILE* f3 = fopen((entity2vecFile+version).c_str(),"w");
                for (int i=0; i<relation_num; i++)
                {
                    for (int ii=0; ii<n; ii++)
...
Train prepare(string root)
{
    Train train;
    string entityFile = root+"/entity2id.txt";
    string relationFile = root+"/relation2id.txt";
    const char *entityFile_str = entityFile.c_str();
    const char *relationFile_str = relationFile.c_str();
    FILE* f1 = fopen(entityFile_str,"r");
	FILE* f2 = fopen(relationFile_str,"r");
	int x;
    ...
    cout<<root<<endl;
    string trainFile = root+"/GI.txt";
    const char *trainFile_str = trainFile.c_str();
    FILE* f_kb = fopen(trainFile_str,"r");
...
int main(int argc,char**argv)
{
    ...
    Train train2;
    train2 = prepare(folderName);
    train2.run(n,rate,margin,method, folderName);
}
